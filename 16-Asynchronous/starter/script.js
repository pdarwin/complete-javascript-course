    ...
    .then(response => {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          ...
          //   whereAmI(lat, lgn);
          // }, 350);
        } else throw new Error(`Whatever (${response.status})`);
      }

      return response.json();
    })
    .then(data => {
      console.log(`Estou em ${data.city}, ${data.country}`);
...
const p = fetch("https://restcountries.com/v3.1/name/guatemala");
console.log(p);
